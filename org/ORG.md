...
Outros pontos de extensão da organização: `org/workflows/` (workflow próprio ou override
de um do pack), `org/professions/`, `org/providers/` — ver `docs/ARCHITECTURE.md` §3.
...
|---|---|---|
| Doc consolidado de issue | `{HU\|HT}{ID}_{TOKEN}_{NomeCurto}.md` | `outputs/{ID}_{NomeCurto}/` |
| `.docx` gerado | mesmo nome, `.docx` | mesma pasta (fora do Git) |
| Discovery | `YYYY-MM-DD_discovery_issue-NNN.md` (ou `_{slug}`) | `history/discoveries/` |
| Análise/priorização | `YYYY-MM-DD_<tipo>_<escopo>.md` | `history/analyses/` |
| Registro de operação (wiki, sprint doc, design) | `YYYY-MM-DD_<tipo>_<slug>.md` | `history/` |
| Export de dados | `<tipo>_YYYY-MM-DD.csv` (nunca sobrescrever dia anterior) | `data/` |
...
- `outputs/`: só `.md` entra no Git; `.docx` e prints são regeneráveis, ficam fora.
- `history/` e `data/`: entram no Git (memória do projeto).
...
- Pipeline de documentação com portões humanos: discovery → `.md` consolidado (**para
  para revisão**) → `.docx` só sob pedido explícito. `.docx` errado → conserta-se o `.md`
  e regera; nunca editar `.docx` à mão.
