...
description: >
  Prioriza issues do backlog GitLab pelo funil MoSCoW → quadrante I×E → ICE score.
  Exporta dados via glab api --paginate, extrai ICE da descrição, ranqueia por MUST/
  ...
  Acione SEMPRE que o usuário mencionar: priorização, priorizado, priorizar, ranking,
  ranqueamento, lista ranqueada, ordem de prioridade, backlog priorizado, funil MoSCoW,
  ice score, quadrante I×E, quais issues entram primeiro, arquivo de priorização,
  history/analyses/priorizacao, anomalia de prioridade, inconsistência de label, ou
  qualquer pedido que combine GitLab + MoSCoW + ICE + ordenar + analisar backlog.
...
backlog, extrai ICE das descrições, aplica o funil de priorização, detecta anomalias abertas
contra o documento de regras do projeto, e gera o markdown priorizado + CSV no disco.

---

## 0. Antes de começar — leia o documento de priorização

**Obrigatório a cada execução.** Leia `docs/context_docs/sistema_priorizacao_funcionamento.md`
e extraia:

- **Fórmula ICE:** dimensões, escala, fórmula (ex: I × C × E)
- **Thresholds da Matriz:** cortes exatos de I e E que definem cada quadrante
- **Quadrantes:** nome, regra, label GitLab correspondente
- **Hierarquia MoSCoW:** qual categoria sempre precede outra
- **Tipos especiais:** categorias que têm fila separada (ex: PRODUCT, REFINAMENTO)
- **Critérios de anomalia:** o que o documento considera inconsistência

Aplique exatamente o que o documento descreve — não assuma valores fixos. Se o documento
mudar thresholds ou fórmulas, sua análise reflete automaticamente.

---

## 1. Determinar escopo

Infira do pedido do usuário ou pergunte quando ambíguo:

| Parâmetro | Padrão | Exemplos alternativos |
|---|---|---|
| Labels de filtro | `BACKLOG` + `BACKLOG (PRIORIZADO)` | "só BACKLOG", "sprint X", label customizada |
| Milestone | nenhum | "sprint 2026.12", milestone específica |
| State | `opened` | "abertas e fechadas" |
...
**Ref.:** [`sistema_priorizacao_funcionamento.md`](docs/context_docs/sistema_priorizacao_funcionamento.md)
...
O arquivo `history/analyses/2026-05-22_priorizacao_backlog.md` é a referência canônica de
output desejado. Consulte-o sempre que houver dúvida sobre formatação, estrutura ou tom.

---

## Notas operacionais

- **Variáveis de ambiente:** ler de `.env` via `export $(grep -v '^#' .env | grep -v '&' | xargs)`  
  ou setar manualmente: `GITLAB_HOST`, `GITLAB_URI`, `GITLAB_TOKEN`, `GITLAB_REPO`
- **CSV com data no nome:** nunca sobrescrever dia anterior — sempre `issues_YYYY-MM-DD.csv`
- **A skill não corrige:** apenas identifica. Correções de labels/descrições no GitLab são
  passo separado e deliberado pelo usuário
- **Milestone opcional:** se o usuário pedir análise de uma sprint específica, adicionar
  `&milestone=Sprint+XXXX` na chamada e ajustar o filtro de labels conforme necessário
