...
- **Uma fase = um registro**: comentário na demanda (marcador `[D1a]`/`[D1b]`/`[D2a]`/
  `[D2b]`) + bloco append em `history/discoveries/YYYY-MM-DD_discovery_<ref>.md`. A
...
D2b completo → aponte o próximo passo (`doc-consolidator`). Sessão encerrada no meio →
rodapé no history com a última fase e as pendências abertas.
